...
{ // checks if x is either tail or head
...
void push_backN(pList p, int N)
{
	...
	if (N == true)
	{
		for (int i = 0; i < N; i++)
		{
			int rand = rand_extended() % (range + 1);
			push_back(p, rand);
		}
	}
	else
...
bool sorted(pList p, bool (*comp)(int a, int b))
{
	...
	int N = size(p);
	pNode curr = begin(p);
	pNode data = begin(p)->next;
	for (int i = 0; i < N; i++)
	{
		if (comp(curr->data, data->data))
			return false;
		else
			return true;
	}
